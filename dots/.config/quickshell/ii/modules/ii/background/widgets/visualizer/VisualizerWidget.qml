...
AbstractBackgroundWidget {
    ...
    Process {
        id: cavaProc
        running: root.isPlaying && Config.options.background.widgets.visualizer.enable
        ...
        stdout: SplitParser {
            onRead: data => {
                root.points = data.split(";").map(point => parseFloat(point.trim())).filter(point => !isNaN(point));
            }
        }
    }
